...
description: "通过 Upmore API 调用字节跳动 Seedance Fast 视频生成模型，支持多模态参考、视频编辑、延长、有声视频、联网搜索增强。"
---

Seedance 2.0 Fast 是字节跳动最新的视频生成模型，通过 Upmore API 提供。支持文生视频、图生视频、多模态参考输入、视频编辑、视频延长及同步音频生成。
...
> [!WARNING]
> 通过 Upmore 网关（`/v1/video/generations`）调用时，下方示例的请求体不能直接使用——顶层 `content`/`duration` 等属于网关封装字段。请把带 `role` 的图片、`duration` 及全部生成参数放进 `metadata`，而不是使用顶层 `images`。完整格式与避坑参见 [Seedance 2.0 — 通过 `metadata` 透传](/zh/guides/model-api/bytedance/seedance-2-0#通过-metadata-透传)。
...
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `model` | string | 是 | `doubao-seedance-2-0-fast` |
