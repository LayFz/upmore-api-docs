---
title: "FLUX.2 [flex]"
description: "通过 Upmore API 调用 Black Forest Labs FLUX.2 [flex] 文生图模型。"
---

FLUX.2 [flex] 是 Black Forest Labs 推出的高度可配置文生图模型，通过 Upmore 提供。提供精细的采样步数和引导系数控制，适合设计专家使用。API 为**同步调用** — 直接返回 Base64 编码的图片数据。
...
<Card
  title="API 参考"
  icon="code"
  href="/zh/api-reference/model-api/blackforestlabs/flux-2-flex"
>
