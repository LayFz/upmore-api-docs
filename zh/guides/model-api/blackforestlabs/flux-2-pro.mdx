---
title: "FLUX.2 [pro]"
description: "通过 Upmore API 调用 Black Forest Labs FLUX.2 [pro] 文生图模型。"
---

FLUX.2 [pro] 是 Black Forest Labs 推出的高级文生图模型，通过 Upmore 提供。支持最高 4MP 分辨率和多参考图一致性。API 为**同步调用** — 直接返回 Base64 编码的图片数据。
