...
description: "通过 Upmore API 调用 Black Forest Labs FLUX.1 Kontext [pro] 图片编辑模型。"
---

FLUX.1 Kontext [pro] 是 Black Forest Labs 推出的上下文感知图片编辑模型，通过 Upmore 提供。可以根据文字指令编辑已有图片，也可以直接文生图。API 为**同步调用** — 直接返回 Base64 编码的图片数据。
