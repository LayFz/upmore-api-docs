...
description: "通过 AnyFast API 调用阿里巴巴 Wan2.2 T2V A14B 文生视频模型。"
---

Wan2.2 T2V A14B 是通过 AnyFast 提供的文生视频模型。视频生成为**异步调用** — 提交任务后，通过 `GET /v1/video/generations/{task_id}` 轮询状态，直到 `status` 为 `succeeded`。
