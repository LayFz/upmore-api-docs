...
description: "通过 Upmore API 调用阿里巴巴 Wan2.2 I2V A14B 图生视频模型。"
---

Wan2.2 I2V A14B 是通过 Upmore 提供的图生视频模型。视频生成为**异步调用** — 提交任务后，通过 `GET /v1/video/generations/{task_id}` 轮询状态，直到 `status` 为 `succeeded`。
...
<Card
  title="API 参考"
  icon="code"
  href="/zh/api-reference/model-api/alibaba/wan2.2-i2v-a14b"
>
