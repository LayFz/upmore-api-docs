...
Sora 2 is OpenAI's video generation model, available through Upmore API. It supports text-to-video, image-to-video (first frame reference), and video remix.
...
```bash cURL
curl https://api.upmore.net/v1/videos \
...
response = requests.post(
    "https://api.upmore.net/v1/videos",
...
```bash cURL
curl https://api.upmore.net/v1/videos/video_69b131ea03548190925a6a06febf993b \
  -H "Authorization: Bearer YOUR_API_KEY"
```

```python Python
status = requests.get(
    f"https://api.upmore.net/v1/videos/{task['id']}",
    headers={"Authorization": "Bearer YOUR_API_KEY"}
).json()
...
```bash cURL
curl https://api.upmore.net/v1/videos/video_69b131ea03548190925a6a06febf993b/content \
  -H "Authorization: Bearer YOUR_API_KEY"
```

```python Python
result = requests.get(
    f"https://api.upmore.net/v1/videos/{task['id']}/content",
    headers={"Authorization": "Bearer YOUR_API_KEY"}
).json()
