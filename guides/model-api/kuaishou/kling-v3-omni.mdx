...
Kling V3 Omni is Kuaishou's latest video generation model, available through Upmore API. The Omni model can achieve various capabilities through prompts combined with elements, images, videos, and other content — supporting text-to-video, image-to-video, video editing, multi-shot storyboarding, and synchronized sound generation.
...
curl https://api.upmore.net/kling/v1/videos/omni-video \
...
response = requests.post(
    "https://api.upmore.net/kling/v1/videos/omni-video",
    headers={
...
```bash cURL
curl https://api.upmore.net/kling/v1/videos/text2video/{task_id} \
  -H "Authorization: Bearer YOUR_API_KEY"
```
