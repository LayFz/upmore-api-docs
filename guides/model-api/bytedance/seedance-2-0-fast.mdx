...
description: "ByteDance's Seedance 2.0 Fast video generation model with multimodal reference, video editing, extension, audio generation, and web search."
---

Seedance 2.0 Fast is ByteDance's latest video generation model available through Upmore API. It supports text-to-video, image-to-video, multimodal reference input, video editing, video extension, and synchronized audio generation.
...
> [!WARNING]
> Through the Upmore gateway (`/v1/video/generations`), the example bodies below do not work as-is — top-level `content`/`duration`/etc. are envelope fields. Put images with `role`, `duration`, and all options inside `metadata` instead of top-level `images`. See the full format and pitfalls in [Seedance 2.0 — Passthrough via `metadata`](/guides/model-api/bytedance/seedance-2-0#passthrough-via-metadata).
...
|---|---|---|---|
| `model` | string | Yes | `doubao-seedance-2-0-fast` |
