...
description: "Google's Gemini 3 Pro Preview streaming via Gemini API. Real-time token streaming with SSE."
---

Gemini 3 Pro Preview (Stream) provides streaming text generation using Google's Gemini 3 Pro Preview model via Upmore. Responses are delivered in real-time using Server-Sent Events (SSE).
...
```bash cURL
curl "https://api.upmore.net/v1beta/models/gemini-3-pro-preview:streamGenerateContent?key=YOUR_API_KEY&alt=sse" \
...
url = "https://api.upmore.net/v1beta/models/gemini-3-pro-preview:streamGenerateContent"
...
<Card
  title="API Reference"
  icon="code"
  href="/api-reference/model-api/google/gemini-3-pro-preview-stream"
>
